...
function getMarketState(): MarketState {
  ...
  // NYSE: 13:30–20:00 UTC (10:30–17:00 ET = 13:30–20:00 UTC)
...
function getMarketBadge(state: MarketState): BadgeConfig {
  const config: Record<MarketState, BadgeConfig> = {
    ...
    weekend: {
      label: 'FIN DE SEMANA',
      color: 'bg-zinc-700/50 text-zinc-400 border-zinc-600/30',
    },
  }
  return config[state]
}
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 p-4 max-w-7xl mx-auto">
        {/* Main content — CockpitHome se autogestiona */}
          ...
          >
            ...
            <div className="flex gap-2 flex-wrap">
              {['SPY', 'BTC', 'GGAL'].map((sym) => (
                <span
                  key={sym}
                  className="text-xs px-2 py-1 rounded bg-zinc-800 text-zinc-400 font-mono"
                >
                  {sym}
                </span>
              ))}
