...
import { TradingSessionMap } from '@/components/market/TradingSessionMap'
import {
...
function TickerStrip({
  ...
  const dollarItems = dollar.slice(0, 3).map(d => ({
    label: `USD ${d.nombre}`, value: formatPrice(d.venta, '$'), change: null as number | null,
  }))
  const cryptoItems = crypto.slice(0, 2).map(c => ({
    label: c.symbol, value: `$${c.price.toLocaleString('en-US', { maximumFractionDigits: 0 })}`, change: c.change24h,
  }))
  const usItems = usMarkets.map(m => {
    ...
    return {
      label: m.label,
      value: isForex ? m.price.toFixed(4) : `$${m.price.toLocaleString('en-US', { maximumFractionDigits: decimals })}`,
  ...
  return (
    <div className="bg-zinc-950 border-b border-zinc-800 overflow-hidden">
      <div className="flex items-center gap-0 overflow-x-auto scrollbar-hide">
        ...
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-2 px-4 py-2 border-r border-zinc-800/60 shrink-0">
...
function MacroCard({ label, value, sub, change, accent = false }: {
  label: string; value: string; sub?: string; change?: number | null; accent?: boolean
}) {
  ...
  return (
    <div className={`rounded-xl border p-4 ${accent ? 'border-amber-500/30 bg-amber-500/5' : 'border-zinc-800 bg-zinc-900'}`}>
      ...
      <p className={`text-2xl font-bold tabular-nums ${accent ? 'text-amber-400' : 'text-white'}`}>{value}</p>
      {change != null ? (
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          {drivers && Array.isArray(drivers) && drivers.length > 0 && (
            <p className="text-xs text-zinc-500 mt-1 line-clamp-1">{drivers[0]}</p>
          )}
        </div>
...
function CategoryPill({ label, href, color }: { label: string; href: string; color: string }) {
  return (
    <a href={href} className="flex items-center justify-between px-3 py-2.5 rounded-lg border border-zinc-800 hover:border-zinc-700 hover:bg-zinc-800/50 transition-all group">
...
export default async function AlDiaPage() {
  ...
  const [{ data: briefings }, latest, dollar, crypto, usMarkets] = await Promise.all([
    supabase.from('briefings').select('id, titulo, tipo, created_at, drivers').order('created_at', { ascending: false }).limit(5),
  ...
  const btc = crypto[0]
  const eth = crypto[1]
  const brecha = blue?.venta && oficial?.venta ? (((blue.venta - oficial.venta) / oficial.venta) * 100) : null
        ...
        <div className="flex items-end justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-semibold text-amber-400 uppercase tracking-wider">INBIG Finanzas</span>
            </div>
          ...
          <div className="flex items-center gap-1.5 text-xs text-zinc-500">
            ...
            <span>Actualizado a las {now.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' })}</span>
          </div>
        </div>

        {/* MacroPills LATAM */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {blue && <MacroCard label="Dólar Blue" value={formatPrice(blue.venta, '$')} sub="venta" accent />}
          {mep && <MacroCard label="Dólar MEP" value={formatPrice(mep.venta, '$')} sub="venta" />}
          {brecha != null && <MacroCard label="Brecha Blue/Oficial" value={`${brecha.toFixed(1)}%`} sub="vs oficial" change={brecha} />}
          {btc && <MacroCard label="Bitcoin" value={`$${btc.price.toLocaleString('en-US', { maximumFractionDigits: 0 })}`} change={btc.change24h} sub="24h" />}
          {eth && <MacroCard label="Ethereum" value={`$${eth.price.toLocaleString('en-US', { maximumFractionDigits: 0 })}`} change={eth.change24h} sub="24h" />}
        </div>

        {/* MacroPills US Markets */}
        {usMarkets.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {usMarkets.map(m => {
              const isForex = m.symbol === 'EURUSD=X'
              const decimals = isForex ? 4 : m.price > 1000 ? 0 : 2
              const formatted = isForex ? m.price.toFixed(4) : `$${m.price.toLocaleString('en-US', { maximumFractionDigits: decimals })}`
              return <MacroCard key={m.symbol} label={m.label} value={formatted} change={m.changePercent} sub="24h" />
            })}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <CopilotBox
              ...
              suggestions={['¿Qué pasó hoy en Argentina?', 'Situación del dólar esta semana', 'Impacto de la Fed en mercados LATAM', 'Bitcoin hoy — análisis rápido']}
            />
            ...
            {briefings && briefings.length > 0 && (
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                <div className="px-5 py-4 border-b border-zinc-800 flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-zinc-500 bg-zinc-800 px-2 py-0.5 rounded-full">Generado por WF3</span>
                </div>
                <div>
            ...
            {featured.length > 0 && (
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {featured.slice(0, 2).map(article => <ArticleCard key={article.id} article={article} variant="featured" />)}
                </div>
              </section>
            )}
            ...
            <section>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden divide-y divide-zinc-800">
                {feed.length > 0
                  ? feed.map(article => <ArticleCard key={article.id} article={article} variant="compact" />)
                  : <div className="px-5 py-12 text-center text-zinc-500 text-sm">Sin noticias aún. WF1 genera contenido cada 30 min.</div>}
              </div>
            </section>
          </div>
                ...
                {dollar.slice(0, 7).map(d => (
                  <div key={d.nombre} className="px-4 py-2.5 flex items-center justify-between">
                    ...
                    <div className="flex gap-3 tabular-nums">
                      <span className="text-xs text-zinc-500">{d.compra != null ? `$${d.compra.toLocaleString('es-AR')}` : '—'}</span>
                      <span className="text-xs font-semibold text-white">{d.venta != null ? `$${d.venta.toLocaleString('es-AR')}` : '—'}</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {crypto.slice(0, 6).map(c => (
                  <div key={c.id} className="px-4 py-2.5 flex items-center justify-between">
                    ...
                    <div className="text-right tabular-nums">
                      <p className="text-xs font-semibold text-white">${c.price.toLocaleString('en-US', { maximumFractionDigits: c.price < 1 ? 4 : 0 })}</p>
